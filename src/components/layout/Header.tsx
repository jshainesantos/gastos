...
export function Header({ title, subtitle, left, right }: Props) {
  return (
    <header className="px-5 pt-12 pb-4 flex items-center justify-between gap-2">
      {left && <div className="mt-1 flex-shrink-0">{left}</div>}
      ...
      {right && <div className="mt-1 flex-shrink-0">{right}</div>}
    </header>
  )
}
